...
public class Test {
    ...
    static void filtersalary(Employee[] emps) {
        ...
        Arrays.stream(emps).filter(x -> x.getSalary() > 20000).forEach(y -> System.out.println(y));
        System.out.println("\n");
    }
    ...
    static void designation(Employee[] emps) {
        ...
        Arrays.stream(emps).filter(employee -> employee.getDesignation().equals("CTO") || employee.getDesignation().equals("DCTO")).forEach(y -> System.out.println(y));
    }
    ...
    static void ageSalaryFilter(Employee[] emps) {
        ...
        Arrays.stream(emps).filter(y -> y.getAge() >= 20 && y.getAge() < 25 && y.getSalary() < 15000).forEach(y -> System.out.println(y));
    }

    //    Filter the employees who are using gmail.
    static void filterGmailUser(Employee[] emps) {
        System.out.println("\n The employees who are using gmail: ");
        Arrays.stream(emps).filter(s -> s.getEmail().contains("gmail")).forEach(y -> System.out.println(y));
    }
...
//    Filter the employee whose mobile number length is not equal to 10.
    static void mobileFilter(Employee[] emps){
        ...
        Arrays.stream(emps).filter(e -> e.getMobile_number().length()!=10).forEach(y -> System.out.println(y));
    }
...
//    Filter the employees whose name starts from “s” and age is greater than 25.
    static void filterNameAge(Employee[] emps){
        ...
        Arrays.stream(emps).filter(y -> y.getName().toLowerCase().startsWith("s") && y.getAge()>25).forEach(y -> System.out.println(y));
    }
